...
import { ActAddAkun, ActGetTypeAkun } from 'components/config/api/finance/kas.api'
  ...
  return (
    <ModalWrapper
      ...
      <div className='p-16px'>
        <TitleModal title={'Tambah Pinjaman'} setisModalClose={setisModalUp} />
      </div>
      ...
      <div style={{
        ...
        <Formik
          ...
          validate={values => {
            ...
            // if (!values?.name) {
            //   errors.name = 'Nama akun tidak boleh kosong'
            // }
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className='p-16px'>
                <div className='d-flex align-center w-100 finance-tambah-akun-informasi-sub-wrapp'>
                  <IInformasi />
                  <div className='finance-tambah-akun-informasi ml-8px'>Informasi</div>
                  <div className='finance-tambah-akun-informasi-sub ml-8px'>Semua nilai yang sudah dibuat tidak dapat diubah kembali</div>
                </div>

                <div className='d-flex flex-column mt-16px gap-8px'>
                  <InputText
                    flexParent={1}
                    marginRightChild={'0px'}
                    title={'Nama Pinjaman*'}
                    placeholder={'Buat Nama Pinjaman'}
                    name={'code'}
                    value={values?.code}
                    handleChange={handleChange}
                  />
                  {/* {errors?.code && <div className='error-phone-number'>{errors?.code}</div>} */}
                ...
                <div className='d-flex flex-column mt-16px gap-8px'>
                  <InputText f
                    ...
                    handleChange={handleChange}
                  />
                  {/* {errors?.name && <div className='error-phone-number'>{errors?.name}</div>} */}
                ...
                <div className='grid grid-cols-2 gap-4'>
                  ...
                  <div style={{ padding: '0px', marginTop: '16px' }} className='w-100 d-flex flex-column gap-10px auth-kontainer-wrapp-input'>
                    <InputDropdownvalue
                      flexParent={1}
                      title={'Type Kalkulasi*'}
                      classTitle={'label-auth'}
                      data={[
                ...
                <div className='grid grid-cols-2 gap-4'>
                  ...
                  <div style={{ padding: '0px', marginTop: '16px' }} className='w-100 d-flex flex-column gap-10px auth-kontainer-wrapp-input'>
                    <InputDropdownvalue
                      flexParent={1}
                      title={'Batas Berlaku denda setelah tenor berahir (Bln)*'}
                      classTitle={'label-auth'}
                      data={[
                        { label: '0', value: '0' },
                      ]}
                ...
                {/* <div style={{ padding: '0px', marginTop: '16px' }} className='w-100 d-flex flex-column gap-10px auth-kontainer-wrapp-input'>
                  <InputDropdownvalue
                    flexParent={1}
                    title={'Diambil Dari Akun*'}
                    classTitle={'label-auth'}
                    data={[
                      { label: 'Pinjaman Reguler', value: 'Pinjaman Reguler' },
                    ]}
                    value={currency}
                    setValue={setcurrency}
                  />
...
export default ModalAddPengaturan