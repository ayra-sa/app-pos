...
export default function HeaderStats() {
  ...
  const isDropdownAkunReducer = useSelector(
    (state) => state.isDropdownAkunReducer
  );
      ...
      <div className="Menu-Navbar-wrapper">
        <div className="Menu-Navbar-wrapper-left">
          <Link href={"/"}>
            <div
              onClick={() => {
                dispatch({ type: "STATUS_SELECT_MENU_RESET_ALL" });
                dispatch({ type: "IS_PAGE_AKUN_FALSE" });
                dispatch({ type: "IS_DROPDOWN_AKUN_FALSE" });
              }}
              className="sidebar-menu-icon-wrapper-home"
            >
              <HomeNavbar />
            </div>
          </Link>
          {menuMasterReducer?.data?.map((res, i) => {
            if (menuMasterReducer?.data?.length - 1 == i) {
              return "";
            }
            return (
              <>
                <Link key={i} href={res?.link}>
                  <div
                    ...
                    className="Menu-Navbar-master-wrapp"
                  >
                    {icons[res?.icon]}
                    <div className="finance-menu-navbar-title">
                      {res?.title}
                    </div>
                    {selectMenuMasterReducer.menu.id === res.id && (
                      <div className="Menu-Navbar-master-wrappActive"></div>
                    )}
                  </div>
                </Link>
        ...
        <div className="Menu-Navbar-wrapper-right">
          <div
            onClick={() => setisBantuanShow(true)}
            className="cursor-pointer Menu-Navbar-wrapper-right-sub"
          >
          ...
          <div
            onClick={() => dispatch({ type: "IS_DROPDOWN_AKUN" })}
            className="cursor-pointer Menu-Navbar-wrapper-right-sub"
          >
            <div style={{ cursor: "pointer" }}>
              <Isetting />
